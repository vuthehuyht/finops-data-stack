...
SELECT
  ...
  CASE
    ...
    ELSE NULLIF(NULLIF(LOWER(DATE), 'nan'), '')::DATE
  END AS DATE,
  CASE
    WHEN EXCHANGE_RATE = '' OR EXCHANGE_RATE = 'None' OR EXCHANGE_RATE IS NULL THEN NULL
    ELSE NULLIF(NULLIF(LOWER(EXCHANGE_RATE), 'nan'), '')::NUMERIC(38, 4)
  END AS EXCHANGE_RATE,
  {{ datacore_common_metadata() }}
FROM {{ latest_source(source("RAW", "RAW_EXCHANGE_RATES"), ['PAIR','DATE']) }}
