...
INCOME_LATEST AS (
  SELECT TICKER, REVENUE, GROSS_PROFIT, OPERATING_PROFIT, NET_PROFIT_AFTER_TAX, REVENUE_YOY, NET_PROFIT_YOY
  FROM (
    SELECT
      TICKER,
      REVENUE,
...
BALANCE_WITH_YOY AS (
  SELECT TICKER, TOTAL_ASSETS, CURRENT_ASSETS, CASH, TOTAL_LIABILITIES, SHORT_TERM_DEBT, LONG_TERM_DEBT, EQUITY, TOTAL_ASSETS_YOY
  FROM (
    SELECT
      TICKER,
      TOTAL_ASSETS,
...
SELECT
  ...
  CASE
    WHEN I.NET_PROFIT_AFTER_TAX != 0 AND CP.OUTSTANDING_SHARE IS NOT NULL
      THEN (R.CLOSE * CP.OUTSTANDING_SHARE) / I.NET_PROFIT_AFTER_TAX
  END::NUMERIC(38, 4) AS PE_RATIO,
  CASE
    WHEN B.EQUITY != 0 AND CP.OUTSTANDING_SHARE IS NOT NULL
      THEN (R.CLOSE * CP.OUTSTANDING_SHARE) / B.EQUITY
  END::NUMERIC(38, 4) AS PB_RATIO,
  -- Cash flow ratios
  CASE
    WHEN I.NET_PROFIT_AFTER_TAX != 0
      THEN CF.CFO / I.NET_PROFIT_AFTER_TAX
  END::NUMERIC(18, 6) AS OPERATING_CASH_FLOW_TO_NET_INCOME,
  (CF.CFO - CF.CAPEX)::NUMERIC(38, 4) AS FREE_CASH_FLOW,
  CASE
