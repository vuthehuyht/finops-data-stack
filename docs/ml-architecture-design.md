...
*   **Dự báo (Inference):** **AWS SageMaker Serverless Inference**. 
    *   *Lợi ích:* Tự động cấp phát tài nguyên khi có request và scale-down về 0 khi không sử dụng. Tối ưu chi phí cho pipeline chạy Daily (chỉ tốn tiền trong vài phút xử lý mỗi ngày).
    *   *Cấu hình dự kiến:* Memory 4GB - 6GB, Max Concurrency tùy tải.
...
*   **CI/CD for ML (MLOps):** Quy trình cập nhật mô hình sẽ dựa trên việc trỏ Serverless Endpoint tới path S3 của phiên bản mới nhất đã qua kiểm định.