...
2.  **Metadata quản trị:** Các trường hệ thống (Metadata) sẽ có **Kiểu dữ liệu chính xác** (Date, Timestamp, Integer) để phục vụ việc phân vùng (Partitioning) và quản lý vận hành.
...
| `raw_financial_ratios` | `ticker`, `date`, `shares_outstanding`, `market_cap` | `vnstock` | Daily / Hàng Quý |
...
| Tên Bảng | Thuộc tính (Schema - All Bronze are String) | Nguồn Khuyến Nghị | Tần suất |
| :--- | :--- | :--- | :--- |
| `raw_macro_indicators` | `indicator_name`, `report_date`, `value`, `unit` | GSO (Tổng cục thống kê) / IMF | Hàng Tháng / Quý |
| `raw_interest_rates` | `rate_type`, `date`, `rate_value` | SBV (NHNN) / Investing.com | Daily |
| `raw_exchange_rates` | `pair`, `date`, `exchange_rate` | Vietcombank API / Investing | Daily |
| `raw_commodities_price`| `commodity_name`, `date`, `price` | Investing.com API / Yahoo Finance API | Daily |
